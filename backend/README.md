...
- Python 3.10+
- MongoDB Community Server ติดตั้งแล้ว และต้องเปิดทำงานอยู่ตลอด (รันที่ `localhost:27017`)
...
**2. ติดตั้ง Django และสร้างโปรเจกต์**
...
**3. สร้าง App สำหรับจัดการสินค้า**

```cmd
python manage.py startapp products
```

**4. ติดตั้ง Package เสริม**
...
**5. แก้ไขไฟล์ `ProjectSIDev/settings.py`**
...
เพิ่มใน `MIDDLEWARE` (บรรทัดบนสุด):

```python
"corsheaders.middleware.CorsMiddleware",
```

เพิ่มบรรทัดใหม่ท้ายไฟล์:

```python
CORS_ALLOWED_ORIGINS = ["http://localhost:5173"]
```

**6. สร้างไฟล์ `db_connection.py`**

database ชื่อ `projectSIDev` collection ชื่อ `products`

**7. เขียน `views.py` และ `urls.py` ในโฟลเดอร์ products**

เขียนโค้ดเองเพื่อกำหนด endpoint `/products/` รองรับ create, list, retrieve, update, delete

**8. ตรวจ MongoDB แล้วรันเซิร์ฟเวอร์**

⚠️ ก่อนรันคำสั่งนี้ ต้องเปิด MongoDB ให้ทำงานอยู่ก่อนเสมอ ไม่งั้น backend จะเชื่อมต่อฐานข้อมูลไม่ได้
...
เปิด `http://localhost:8000/products/` แล้วเห็นข้อมูล JSON (หรือ `[]` ถ้ายังไม่มีสินค้า)

**9. บันทึกรายการ dependency ทั้งหมด**

```cmd
pip freeze > requirements.txt
```

ได้ไฟล์ `requirements.txt` ไว้ให้คนอื่น `pip install -r requirements.txt` ตอน clone แทนที่จะทำขั้น 1–8 ใหม่ทั้งหมด

---

## MongoDB Compass (ทางเลือก)

โปรแกรมช่วยดูข้อมูลใน MongoDB แบบเห็นภาพ (GUI) แทนการดูผ่านโค้ด
